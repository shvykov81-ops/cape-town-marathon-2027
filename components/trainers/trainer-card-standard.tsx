...
import { Star, MapPin, BookOpen, ArrowUpRight } from "lucide-react";
      ...
      <div className="relative h-full rounded-2xl bg-white/[0.03] border border-white/10 overflow-hidden group hover:border-teal-500/30 transition-colors duration-500">
        ...
        <div className="p-5">
          <Link href={`/${locale}/trainers/${trainer.slug}`}>
            <h3 className="text-lg font-bold text-white group-hover:text-teal-400 transition-colors">{name}</h3>
          </Link>
          {trainer.headline && (
          ...
          <div className="flex flex-wrap gap-1.5 mt-3">
            {trainer.specialties.slice(0, 3).map((s) => (
              <span key={s} className="px-2 py-1 rounded-full text-xs bg-white/5 text-white/60 border border-white/10">{s}</span>
            ))}
          </div>
          ...
          {/* ─── FIX: Book with Trainer CTA ─── */}
          <div className="flex gap-2 mt-4">
            <Link
              href={`/${locale}/trainers/${trainer.slug}`}
              className="flex-1 text-center py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm font-medium text-white hover:bg-white/10 transition-colors flex items-center justify-center gap-1"
            >
              {t("card.viewProfile")}
            ...
            <Link
              ...
              className="flex-1 text-center py-2.5 rounded-xl bg-teal-500 text-sm font-medium text-black hover:bg-teal-400 transition-colors flex items-center justify-center gap-1"
            >
