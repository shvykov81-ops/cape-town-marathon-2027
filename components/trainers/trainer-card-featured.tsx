...
export function TrainerCardFeatured({ trainer, index = 0 }: TrainerCardFeaturedProps) {
  ...
  return (
    ...
    >
      <div className="group block relative h-full rounded-3xl bg-white/[0.03] border border-white/10 overflow-hidden hover:border-teal-500/30 transition-all duration-500">
        <Link href={`/${locale}/trainers/${trainer.slug}`} className="block relative aspect-[16/10] md:aspect-auto md:h-full overflow-hidden">
          {imageUrl ? (
            <Image
              src={imageUrl}
              alt={name}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
            <div className="flex items-end justify-between gap-4">
              <div>
                <h3 className="text-2xl md:text-3xl font-bold text-white group-hover:text-teal-400 transition-colors">{name}</h3>
                {trainer.experienceYears && (
                  <p className="text-white/60 mt-1">{trainer.experienceYears}+ {t("card.years")}</p>
                )}
                {trainer.headline && (
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {trainer.specialties.slice(0, 4).map((s) => (
                    <span key={s} className="px-3 py-1 rounded-full text-sm bg-white/10 text-white/80 border border-white/10">{s}</span>
                  ))}
                </div>
              ...
              {/* ─── FIX: Book with Trainer CTA for Featured Card ─── */}
              <div className="hidden md:flex flex-col gap-2">
                <Link
                  href={`/${locale}/trainers/${trainer.slug}`}
                  className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-white/10 backdrop-blur-md text-white font-semibold hover:bg-white/20 transition-colors"
                >
                  {t("card.viewProfile")}
