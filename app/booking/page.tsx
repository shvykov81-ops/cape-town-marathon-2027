...
import {
  Check, ChevronRight, ChevronLeft, Star, Shield, Lock, User, Package, Loader2,
} from "lucide-react";

interface Trainer {
  id: string; slug: string; firstName: string; lastName: string;
  displayName: string | null; headline: string | null;
  photoUrl: string | null; photos: string[];
  specialties: string[]; languages: string[];
  experienceYears: number | null; rating: number; reviewCount: number;
}

interface PackageData {
  id: string; name: string; durationDays: number;
  priceBase: string; description: string; includes: string[];
}

interface PackageOption {
  id: string; name: string; priceAdd: string; category: string;
}
        ...
        return (
          <div key={label} className="flex items-center gap-2">
            <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-semibold transition-colors ${
              isDone ? "bg-teal-500 text-black" : isActive
            ...
            {i < steps.length - 1 && (
              <div className={`w-8 h-px mx-1 ${isDone ? "bg-teal-500/40" : "bg-white/10"}`} />
            )}
          </div>
        );
      })}
...
function TrainerSelectCard({
  trainer, selected, onSelect, t, locale,
}: {
  trainer: Trainer; selected: boolean; onSelect: () => void;
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {trainer.headline && <p className="text-sm text-white/50 truncate">{trainer.headline}</p>}
          <div className="flex items-center gap-2 mt-1.5">
            ...
            <span className="text-sm text-white/50">{trainer.reviewCount} {t("list.reviews")}</span>
            {trainer.experienceYears && (
              <><span className="text-white/30">·</span>
              <span className="text-sm text-white/50">{trainer.experienceYears}+ yrs</span></>
            )}
          </div>
          <div className="flex flex-wrap gap-1 mt-2">
            {trainer.specialties.slice(0, 2).map((s) => (
              <span key={s} className="px-2 py-0.5 rounded-full text-xs bg-white/5 text-white/50 border border-white/10">{s}</span>
            ))}
          </div>
        </div>
      </div>
      <Link href={`/${locale}/trainers/${trainer.slug}`} onClick={(e) => e.stopPropagation()}
        className="inline-flex items-center gap-1 mt-3 text-sm text-teal-400 hover:text-teal-300 transition-colors">
        {t("card.viewProfile")}<ChevronRight className="w-4 h-4" />
      </Link>
    </motion.div>
  );
}
...
export default function BookingPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    dob: "", checkInDate: "", checkOutDate: "", raceCategory: "",
  });

  useEffect(() => {
    fetch("/api/packages").then(r => r.json()).then(data => {
      setPackages(data.packages || []); setLoadingPackages(false);
  ...
  useEffect(() => {
    ...
    fetch("/api/trainers?limit=50&sortBy=rating").then(r => r.json()).then(data => {
      setTrainers(data.trainers || []); setLoadingTrainers(false);
  ...
  useEffect(() => {
    if (!selectedPkg) return;
    fetch(`/api/packages/${selectedPkg.id}`).then(r => r.json())
      .then(data => setOptions(data.options || [])).catch(() => setOptions([]));
  }, [selectedPkg]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/booking", {
        ...
        body: JSON.stringify({
          packageId: selectedPkg?.id, trainerId: selectedTrainer?.id,
          firstName: formData.firstName, lastName: formData.lastName,
          email: formData.email, phone: formData.phone, country: formData.country,
          dateOfBirth: formData.dob, checkInDate: formData.checkInDate,
          checkOutDate: formData.checkOutDate, raceCategory: formData.raceCategory,
          extras: selectedExtras,
        }),
      });
      if (!res.ok) throw new Error("Booking failed");
      setStatus("success"); setStep(5);
    } catch {
      setStatus("error"); setErrorMsg("Failed to create booking. Please try again.");
    } finally { setSubmitting(false); }
  };

  const canProceed = () => {
    if (step === 1) return !!selectedPkg;
    if (step === 2) return formData.firstName && formData.lastName && formData.email && formData.checkInDate && formData.checkOutDate;
          ...
          <div className="lg:col-span-2">
            <AnimatePresence mode="wait">
              ...
              {step === 1 && (
                <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  ...
                  {loadingPackages ? (
                    <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-teal-400" /></div>
                  ) : packages.length === 0 ? (
                    <p className="text-white/50 text-center py-12">{t("loading")}</p>
                  ) : (
                    <div className="grid sm:grid-cols-2 gap-4">
                      {packages.map(pkg => (
                        <motion.div key={pkg.id} whileHover={{ y: -4 }} onClick={() => setSelectedPkg(pkg)}
                          ...
                          <div className="flex items-center justify-between mb-3">
                            ...
                            {selectedPkg?.id === pkg.id && (
                              <div className="w-6 h-6 rounded-full bg-teal-400 flex items-center justify-center"><Check className="w-4 h-4 text-black" /></div>
                            )}
                          </div>
                          <p className="text-white/50 text-sm mb-4">{pkg.description}</p>
                          <div className="flex items-baseline gap-1">
                            <span className="text-3xl font-bold text-teal-400">${Number(pkg.priceBase).toLocaleString()}</span>
                            <span className="text-white/40">/ {pkg.durationDays} {t("days")}</span>
                          </div>
                          <ul className="mt-4 space-y-2">
                            {pkg.includes.slice(0, 4).map(inc => (
                              <li key={inc} className="flex items-center gap-2 text-sm text-white/60"><Check className="w-4 h-4 text-teal-400" />{inc}</li>
                            ))}
                          </ul>
                        </motion.div>
                      ))}
              ...
              {step === 2 && (
                <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4 mb-8">
                    {[
                      ...
                      <div key={field.key} className={field.type === "date" ? "sm:col-span-1" : ""}>
                        ...
                        <input type={field.type} value={formData[field.key as keyof typeof formData]}
                  ...
                  <div className="mb-8">
                    ...
                    <select value={formData.raceCategory}
                  ...
                  <div className="mb-8">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-xl font-bold flex items-center gap-2">
                        <User className="w-5 h-5 text-teal-400" />{t("selectTrainer")}
                      </h3>
                      {selectedTrainer && (
                        <button onClick={() => setSelectedTrainer(null)}
                          className="text-sm text-white/40 hover:text-white/70 transition-colors">Clear selection</button>
                      )}
                    </div>
                    {loadingTrainers ? (
                      <div className="flex items-center justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-teal-400" /></div>
                    ) : trainers.length === 0 ? (
                      <p className="text-white/40 text-center py-8 bg-white/5 rounded-2xl border border-white/10">No trainers available at the moment.</p>
                    ) : (
                      <div className="grid gap-3">
                        {trainers.map(trainer => (
                          <TrainerSelectCard key={trainer.id} trainer={trainer}
                            selected={selectedTrainer?.id === trainer.id}
                            onSelect={() => setSelectedTrainer(prev => prev?.id === trainer.id ? null : trainer)}
                            t={t} locale={locale} />
                        ))}
                      </div>
                    )}
              ...
              {step === 3 && (
                <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  ...
                  {options.length === 0 ? (
                    <p className="text-white/50 text-center py-12 bg-white/5 rounded-2xl border border-white/10">{t("noExtras")}</p>
                  ) : (
                    <div className="space-y-3">
                      {options.map(option => (
                        <motion.div key={option.id} whileHover={{ x: 4 }} onClick={() => toggleExtra(option.id)}
                          className={`flex items-center justify-between p-5 rounded-2xl border-2 cursor-pointer transition-all ${
                            selectedExtras.includes(option.id)
                              ? "border-teal-400 bg-teal-400/10" : "border-white/10 bg-white/5 hover:border-white/20"
                          }`}>
                          <div className="flex items-center gap-4">
                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${
                              selectedExtras.includes(option.id) ? "border-teal-400 bg-teal-400" : "border-white/20"
                            }`}>
                              {selectedExtras.includes(option.id) && <Check className="w-4 h-4 text-black" />}
                            </div>
                            <div>
                          ...
                          <span className="text-teal-400 font-semibold">+${Number(option.priceAdd).toLocaleString()}</span>
                        </motion.div>
                      ))}
                    </div>
                  )}
                  ...
                  <div className="space-y-4">
                    <div className="p-5 rounded-2xl bg-white/5 border border-white/10">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-teal-400 font-bold">${Number(selectedPkg?.priceBase).toLocaleString()}</span>
                      </div>
                      <p className="text-sm text-white/50 mt-1">{selectedPkg?.durationDays} days</p>
                    </div>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="relative w-12 h-12 rounded-xl overflow-hidden bg-gradient-to-br from-teal-500/20 to-amber-500/20">
                            {(selectedTrainer.photos?.[0] || selectedTrainer.photoUrl) ? (
                              <Image src={selectedTrainer.photos?.[0] || selectedTrainer.photoUrl!}
                                alt={`${selectedTrainer.firstName} ${selectedTrainer.lastName}`}
                                fill className="object-cover" sizes="48px" />
                            ) : (
                              <div className="absolute inset-0 flex items-center justify-center font-bold text-white/60">{selectedTrainer.firstName[0]}</div>
                            )}
                          </div>
                          <div>
                            <p className="font-semibold text-white">{selectedTrainer.displayName || `${selectedTrainer.firstName} ${selectedTrainer.lastName}`}</p>
                            <div className="flex items-center gap-1 text-sm text-white/50">
                              <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />
                              {selectedTrainer.rating.toFixed(1)} · {selectedTrainer.reviewCount} {t("list.reviews")}
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="p-5 rounded-2xl bg-white/5 border border-white/10">
                      ...
                      <p className="text-white/80">{formData.firstName} {formData.lastName}</p>
                      <p className="text-white/50 text-sm">{formData.email}</p>
                      <p className="text-white/50 text-sm mt-1">{formData.checkInDate} → {formData.checkOutDate}</p>
                    </div>
                    ...
                    <button onClick={handleSubmit} disabled={submitting}
                      ...
                      {submitting ? (
                        ...
                        <><Lock className="w-5 h-5" />{t("confirm")} — ${total.toLocaleString()}</>
                      )}
                    </button>

                    <div className="flex items-center justify-center gap-4 text-xs text-white/30">
                      <span className="flex items-center gap-1"><Shield className="w-3 h-3" /> {t("ssl")}</span>
                      <span className="flex items-center gap-1"><Lock className="w-3 h-3" /> {t("encrypted")}</span>
                    </div>
                  </div>
                </motion.div>
              )}
                ...
                <motion.div key="step5" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <Link href={`/${locale}/dashboard`}
                      className="px-8 py-3 rounded-xl bg-teal-500 text-black font-semibold hover:bg-teal-400 transition-colors">{t("goDashboard")}</Link>
                    <Link href={`/${locale}`}
                      className="px-8 py-3 rounded-xl bg-white/5 border border-white/10 text-white font-semibold hover:bg-white/10 transition-colors">{t("backHome")}</Link>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {step < 5 && (
              <div className="flex items-center justify-between mt-8">
                ...
                <button onClick={() => setStep(s => Math.min(5, s + 1))} disabled={!canProceed()}
                  ...
                  {step === 4 ? t("reviewBtn") : t("continue")}<ChevronRight className="w-5 h-5" />
                </button>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 rounded-2xl bg-white/[0.03] border border-white/10 p-6">
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                <Package className="w-5 h-5 text-teal-400" />{t("orderSummary")}
              </h3>
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-teal-400/5 border border-teal-500/20 mb-4">
                  <div className="relative w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-gradient-to-br from-teal-500/20 to-amber-500/20">
                    {(selectedTrainer.photos?.[0] || selectedTrainer.photoUrl) ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center text-sm font-bold text-white/60">{selectedTrainer.firstName[0]}</div>
                    )}
                  </div>
                  <div>
                    <p className="text-xs text-teal-400/60">{t("yourTrainer")}</p>
                    <p className="font-medium text-sm">{selectedTrainer.displayName || `${selectedTrainer.firstName} ${selectedTrainer.lastName}`}</p>
                  </div>
                </div>
              )}
              ...
              {selectedPkg && (
                <div className="flex justify-between py-3 border-b border-white/10">
                  <div>
                    ...
                    <p className="text-sm text-white/50">{selectedPkg.durationDays} {t("days")}</p>
                  </div>
                  <span className="font-semibold text-teal-400">${Number(selectedPkg.priceBase).toLocaleString()}</span>
                </div>
              )}
                  ...
                  {selectedExtras.map(id => {
                    ...
                    return (
                      <div key={id} className="flex justify-between py-1">
                        ...
                        <span className="text-sm text-teal-400">+${Number(extra?.priceAdd).toLocaleString()}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="flex items-center gap-3 mt-4 pt-4 border-t border-white/10 text-xs text-white/30">
                <span className="flex items-center gap-1"><Shield className="w-3 h-3" /> {t("ssl")}</span>
                <span className="flex items-center gap-1"><Lock className="w-3 h-3" /> {t("encrypted")}</span>
                <span className="flex items-center gap-1"><Check className="w-3 h-3" /> {t("verified")}</span>
              </div>
            </div>
          </div>
        </div>
